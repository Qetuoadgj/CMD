...
:Start
	...
	echo.
	if /i {%lang%}=={0419} (echo. ^(1^) - Конвертировать в моно) else (echo. ^(1^) - Convert to mono)
	if /i {%lang%}=={0419} (echo. ^(r^) - Выйти) else (echo. ^(r^) - Exit)

:ChoosePreset
	echo.
	set "ANSWER="
	if /i {%lang%}=={0419} (SET /P "ANSWER=Заготовка: ") else (SET /P "ANSWER=Preset: ")
...
:Set_Input_Channels
	...
	echo.
	set "ANSWER="
	if /i {%lang%}=={0419} (SET /P "ANSWER=Каналов на входе: ") else (SET /P "ANSWER=Input channels: ")
...
:Set_Input_Volume
	cls
	if /i {%lang%}=={0419} (echo.Каналов на входе: %INPUT_CHANNELS%) else (echo.Input channels: %INPUT_CHANNELS%)
	echo.
	set "ANSWER="
...
:Set_Output_Channels
	...
	echo.
	set "ANSWER="
	if /i {%lang%}=={0419} (SET /P "ANSWER=Каналов на выходе: ") else (SET /P "ANSWER=Output channels: ")
...
:Set_Output_Format
	...
	if /i {%lang%}=={0419} (echo.Битрейт: %OUTPUT_BITRATE%) else (echo.BitRate: %OUTPUT_BITRATE%)
	if /i {%lang%}=={0419} (echo.Качество: %OUTPUT_SAMPLE_RATE%) else (echo.Sample rate: %OUTPUT_SAMPLE_RATE%)
	if /i {%lang%}=={0419} (echo.Каналов на выходе: %OUTPUT_CHANNELS%) else (echo.Output channels: %OUTPUT_CHANNELS%)
	echo.
	if /i {%lang%}=={0419} (echo.Выбор действия:) else (echo.Action select:)
	echo.
